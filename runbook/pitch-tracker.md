# Pitch tracker — allocated to analysts, tracked to closure

Open pitches/deal threads allocated to the deal desk on **22 Sep 2026**, each to be driven to
**response closure** (revert to founder + a status back to Pratekk). Pratekk is cc'd on both
allocation emails, so status replies land in his inbox on one thread per analyst; the daily
brief reads those threads and updates **Status** + **Last update** below.

**Status legend:** `allocated` → `contacted` → `call set` → `in diligence` / `passed` (closed).
Closure = a clear outcome (in diligence or passed), not just "seen."

## Neeti (Neeti.B@growthcap.vc) — 7
| # | Pitch | Sector / ask | Allocation action | Status | Last update |
|---|---|---|---|---|---|
| 1 | Acquafront Infrastructure (Anukul, BrightBridge) | Infra · ₹60 Cr pre | Pin valuation/equity or pass (quiet ~15 d) | allocated | 22 Sep |
| 2 | MacHanas (Harshit) | AI procurement for MSMEs · ₹12 Cr | Lock call + revert with view | allocated | 22 Sep |
| 3 | SlagExcel (Samarth, Samsaara) | Sustainable building materials | Take call + revert / pass | allocated | 22 Sep |
| 4 | Seamless/Artemis + Techtronics + Vendor Infra (ExcessEdge) | US$1M seed + 2 more | Clean pass/next on all three | allocated | 22 Sep |
| 5 | ETFly (Preet Parekh) | Fintech · India ETF platform | Screen + revert | allocated | 22 Sep |
| 6 | QuantArena (Niloy Ray) | Financial-intelligence | Screen + revert / pass | allocated | 22 Sep |
| 7 | Agrify (Prateek Garg) | Agri marketplace · ₹25 Cr | Warm pass (off-thesis) | allocated | 22 Sep |

## Smridh (Smridh.K@growthcap.vc) — 7
| # | Pitch | Sector / ask | Allocation action | Status | Last update |
|---|---|---|---|---|---|
| 1 | Okulo Aerospace | Deep-tech / aero | Drive call to view (IC note / pass) | allocated | 22 Sep |
| 2 | Deepterrain.ai (Alok Jain) | Autonomous surveillance | Lock Thu/Fri slot (waiting since 20 Sep) | allocated | 22 Sep |
| 3 | Flaunt (via Pratiti / Riddhi) | Multimodal vision AI (beauty/fashion) | Pick up + take forward | allocated | 22 Sep |
| 4 | ParkoBot (Amrit, gopark) | Parking marketplace | Screen + revert (re-pinged ×2) | allocated | 22 Sep |
| 5 | PolarPro (Dhruv) | Agri AI | Polite pass (off-thesis) | allocated | 22 Sep |
| 6 | 64codon (Dr Shaji) | Deep-tech (met at Pivot Kochi) | Set a call + revert (warm) | allocated | 22 Sep |
| 7 | Impactful Pitch batch (Akshay) | Broker: Vensor, Prospero, Biokraft, Skywall, BlissBody, RisoApp | One "pass, thanks" (flag any keeper) | allocated | 22 Sep |

## Not allocated — noise / no action (logged so they don't resurface as "missed")
JS Pharma (Mark Donald), Cregis crypto gateway, DigiMaaya SAFE, onestaff.ai, Corespan/zaidwood,
Achut Krovvidi (Gen-Z shopping), Basylic (crypto treasury); plus event invites (TiE, FICCI,
Founder Presentation Week) and newsletters (Tracxn, PitchBook, Mainstream).

## How the routine keeps this live
- The two allocation emails (22 Sep) cc Pratekk; each analyst replies **on the same thread**
  with a status per pitch.
- The **daily brief** reads those two threads: for each row, if the analyst has replied since
  allocation, update **Status** + **Last update**; if no reply after the ask, surface it under
  **⏳ Delegated & gone quiet** with the days-pending clock.
- Chase cadence: first status was asked for by **Thu 25 Sep EOD**; flag any row still
  `allocated` (no analyst reply) after that.
